...
export default function Roster({ players = [] }: Props) {
  ...
  // Filter + sort (sort is fixed to Name A–Z)
  const filtered = useMemo(() => {
    const qLower = (q || "").toLowerCase();
    let list = players.filter((p) => (p.strPlayer ?? "").toLowerCase().includes(qLower));
    if (pos !== "All") list = list.filter((p) => (p.strPosition || "Unknown") === pos);
    return list.sort((a, b) => (a.strPlayer || "").localeCompare(b.strPlayer || ""));
  }, [players, q, pos]);
      ...
      <Card className="p-4 sm:p-5 md:p-6 ring-1 ring-white/10 bg-gradient-to-b from-white/5 to-transparent">
        ...
        <div className="flex items-end justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="text-white/70 text-xs md:text-[13px] mt-0.5">Player directory</p>
          </div>
        </div>

        {/* Controls: name search + position chips */}
        ...
        {filtered.length === 0 && (
          <div className="p-8 text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 ring-1 ring-inset ring-white/10">
              <svg viewBox="0 0 24 24" className="h-4.5 w-4.5 text-white/70" fill="currentColor" aria-hidden>
                <path d="M12 2a5 5 0 0 1 5 5v2h1a3 3 0 0 1 3 3v7h-6v-5H9v5H3v-7a3 3 0 0 1 3-3h1V7a5 5 0 0 1 5-5z" />
              </svg>
              <span className="text-sm text-white/70">No players match your search.</span>
            </div>
          </div>
        )}
      </Card>
